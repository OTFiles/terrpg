...
// GameEngine实现
GameEngine::GameEngine() : currentMap("start"), playerX(5), playerY(5), playerDir('d') {}
    ...
    try {
        if(tokens[0] == "add") {
            ...
            if(tokens[1] == "map") {
                ...
                if(tokens.size() > 5) obj.name = tokens[5];
                if(tokens.size() > 6) obj.display = tokens[6][0];
                
                maps[currentMap].setObject(x, y, obj);
            }
            else if(tokens[1] == "npc") {
